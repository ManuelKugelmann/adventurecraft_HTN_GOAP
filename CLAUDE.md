...
Two primitives: `entity.path` and `edge(from, to, type, [depth], [agg])`.
Operators: `+ - * / < > == != >= <= AND OR NOT`.
Functions: `min max abs sigmoid prob random count any sum`.

## Validation Rules

1. All Expressions resolve to valid terminals
2. `do` uses valid Action × Approach (7×3 table)
